...
BZ_Calls.NetEvent('esx_addons_gcphone:startCall',
	function(source, number, message, coords)
		...
		for _, src in ipairs(ESX.GetPlayers()) do
			if src ~= source then
				...
				if player and player.job and player.job.name == number then
					TriggerClientEvent('esx_service_request:call', player.source, id)
					found = true
				end
			end
		end
		...
		TriggerClientEvent('esx_service_request:stop', -1, id)
		TriggerClientEvent('esx:showNotification', source, _U('reject'))
	end)

BZ_Calls.NetEvent('esx_service_request:accept',
	function(source, id)
		local data = calls[id]
		...
		if data then
			...
			TriggerClientEvent('esx_service_request:take', source, data.coords, name)
